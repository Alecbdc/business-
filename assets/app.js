...
function renderSandbox() {
  ...
  const historyMarkup = marketState.history
    .slice(-10)
    .reverse()
    .map((entry) => {
      const toneClass =
  ...
  renderStockDetails(activeAsset);
...
function renderStockDetails(symbol) {
  ...
  if (!detail) {
    ...
    setText($('#stock-summary'), 'Select a stock to load fundamentals.');
    ['stock-metrics', 'stock-events', 'stock-eps-list', 'stock-performance', 'stock-news'].forEach((id) => {
  ...
  setText($('#stock-profile-meta'), `${detail.country} • ${detail.sector}`);
  setText($('#stock-profile-sector'), detail.sector ?? 'Sector');
  setText($('#stock-summary'), detail.summary ?? '');

  const metricsEl = $('#stock-metrics');
  if (metricsEl) {
  ...
  const perfEl = $('#stock-performance');
  if (perfEl) {
        ...
        .map(
          (row) => `
            <div class="flex items-center justify-between rounded-2xl border border-white/5 px-3 py-2">
              <span class="text-xs text-slate-400">${row.period}</span>
              <div class="text-right text-sm">
                <p>Rev growth: ${formatPercent(row.revenueGrowth)}</p>
        ...
        .map(
          (item) => `
            <div class="flex items-start justify-between rounded-2xl border border-white/5 bg-white/5 px-3 py-2">
              ...
              <span class="badge ${item.sentiment === 'positive' ? 'bg-emerald-500/20 text-emerald-100' : 'bg-white/10 text-slate-200'}">${
            item.sentiment ?? 'neutral'
          }</span>
            </div>
          `
        )
        .join('') || '<p class="text-sm text-slate-400">No company-specific news yet.</p>';
  }
}
...
function renderMarketLab() {
  ...
  if (historyContainer) {
    const historyMarkup = state.marketLab.history
      ?.slice(-10)
      .reverse()
      .map((entry) => {
        const toneClass =
  ...
  if (stripContainer) {
    const entries = Object.entries(state.marketLab.prices);
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-eps-period]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      renderStockDetails(currentActiveAsset());
    });
  });
  document.querySelectorAll('[data-fin-period]').forEach((btn) => {
    btn.addEventListener('click', () => {
