...
function findScenario(levelId, scenarioId) {
  ...
  const scenario = level?.scenarios.find((s) => s.id === scenarioId) || null;
            ...
            <div class="rounded-2xl border border-white/10 bg-white/5 p-4 flex flex-col gap-3 scenario-row" data-level="${
              ...
              <div class="flex items-start justify-between gap-3">
                ...
                <div class="flex flex-col items-end gap-1 text-right text-xs text-slate-400">
                  ...
                  <span class="badge ${
                    scenario.segment === 'crypto'
                      ? 'bg-sky-500/20 text-sky-100'
                      : 'bg-amber-500/20 text-amber-100'
                  }">${scenario.segment === 'crypto' ? 'Crypto' : 'Stocks'}</span>
                </div>
              </div>
              <button
...
function resetScenarioSelection() {
  ...
  state.marketScenario.segment = 'stocks';
...
function openScenarioModal(levelId, scenarioId) {
  ...
  const segmentLabel = scenario.segment === 'crypto' ? 'Crypto' : 'Stocks';
  setText($('#scenario-modal-level'), `${level.name} • ${segmentLabel} • ${scenario.durationDays}d window`);
...
function startScenarioSession(levelId, scenarioId, options = {}) {
  ...
  const basePrices = Object.fromEntries(
    segmentSymbols(segment).map((symbol) => [symbol, state.prices[symbol] ?? initialPrices[symbol] ?? 0])
  );
  const seededHistory = seedPriceHistoryMap(initialPrices);
  const filteredHistory = Object.fromEntries(
    Object.entries(seededHistory).filter(([symbol]) => allowed.has(symbol))
  );
  const defaultAsset = (segmentSymbols(segment)[0] ?? assetSymbols[0]) || 'BTC';
...
function renderSandbox() {
  ...
  if (isScenario) {
    const options = allowedSymbols.map((symbol) => `<option value="${symbol}">${symbol}</option>`).join('');
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-eps-period]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      renderAssetDetails(currentActiveAsset(), state.sandboxMode === 'scenario' ? scenarioSegment() : segment, context);
    });
  });
  document.querySelectorAll('[data-fin-period]').forEach((btn) => {
    btn.addEventListener('click', () => {
