...
import { supabaseConfig, featureToggles } from './config.js';
    ...
    .map(
      (entry) => `
        <div class="flex items-center justify-between rounded-2xl bg-white/5 px-3 py-2">
          <span class="text-sm">${entry.lessonTitle}</span>
          <span class="text-sm font-semibold">${entry.score}%</span>
        </div>
      `
    )
    .join('');
...
async function handleQuizSubmit(event) {
  ...
  state.quizLog.push({ lessonId: lesson.id, lessonTitle: lesson.title, score, ts: new Date().toISOString() });
  ...
  showToast(`You scored ${score}%`, passed ? 'success' : 'info');
}
...
function enterDemoMode({ silent = false } = {}) {
...
function handleDemoEntry(event) {
  event?.preventDefault();
  enterDemoMode();
}

function maybeAutoEnterDemo() {
  if (state.user || state.previewMode) return;
  if (!supabaseClient) {
    const hint = $('#demo-hint');
    if (hint) {
      hint.textContent = 'Offline preview: the full workspace is open without Supabase credentials.';
      hint.classList.add('text-emerald-300');
    }
    enterDemoMode({ silent: true });
    showToast('No Supabase configuration detected, loading the offline demo workspace.', 'info');
    return;
  }
}
