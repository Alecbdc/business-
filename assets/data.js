...
export const stockFundamentals = Object.fromEntries(
  stockCompanies.map((company, idx) => {
    ...
    const performanceAnnual = Array.from({ length: 3 }).map((_, i) => {
      ...
      const cash = Number((cap * 0.08 - i * 0.5).toFixed(1));
      return { period, revenueGrowth, netIncome, margin, cash };
    });
    const perfQuarterly = quarterlyLabels.map((period, i) => {
      ...
      const cash = Number((cap * 0.05 - i * 0.3).toFixed(1));
      return { period, revenueGrowth, netIncome, margin, cash };
    });
    return [
      company.symbol,
    ...
    return [
      asset.symbol,
      {
        ...
        metrics: {
          ...
          circulating: Number((cap * 20).toFixed(0))
        },
        events: [
...
export const marketScenarioLevels = [
  {
    ...
    scenarios: [
      ...
      },
      {
        ...
        teaser: 'Crypto rotates from majors into memes and L2s as liquidity drips back.',
        difficulty: 1,
        segment: 'crypto',
        durationDays: 45,
        bias: 0.002,
        ...
        xpRewards: [100, 200, 300],
        description:
          'Inspired by classic crypto mini-altseasons: BTC and ETH grind while pockets of the market rip on narratives and social buzz.',
        tips: [
          'Take partial profits on fast-moving meme coins and rotate back into majors.',
          'Size positions for higher volatility—intraday swings are larger than stocks.',
          'Use dips in majors to rebalance when speculative names overextend.'
        ],
        news: [
          {
            id: 'memes-pop',
            title: 'Meme coins spike as on-chain activity surges',
            summary: 'BONK, DOGE, and SHIB volumes explode after viral threads.',
            drift: 0.16,
          ...
          {
            id: 'l2-fees',
            title: 'Layer-2 fees drop; builders flock to rollups',
            summary: 'Lower gas boosts SOL and AVAX ecosystems while majors consolidate.',
            drift: 0.07,
            assets: ['SOL', 'AVAX', 'ALT_BASKET'],
            atDay: 16
          },
          {
            id: 'rotation-back',
            title: 'Rotation back to majors as traders de-risk',
            summary: 'Flows return to BTC and ETH after meme froth cools.',
            drift: -0.05,
            assets: ['BTC', 'ETH', 'USDT'],
            atDay: 31
          }
  ...
  },
  {
    ...
    scenarios: [
      ...
      },
      {
        ...
        teaser: 'Policy chatter and depegs jolt majors while DeFi volumes jump.',
        difficulty: 2,
        segment: 'crypto',
        durationDays: 55,
        bias: -0.0025,
        ...
        xpRewards: [100, 200, 300],
        description:
          'Inspired by past stablecoin wobbles: brief depegs, liquidity crunches, and fast rotations between majors and stables.',
        tips: [
          'Watch liquidity and spreads when stablecoins wobble—slippage rises.',
          'Keep dry powder in multiple stables to manage conversion risk.',
          'Majors can dip first but often rebound faster than small caps after stress.'
        ],
        news: [
          {
            id: 'policy-probe',
            title: 'Regulators probe reserve disclosures at popular stablecoin',
            summary: 'USDT briefly slips; BTC and ETH catch bids as traders rotate.',
            drift: -0.1,
            assets: ['USDT', 'BTC', 'ETH'],
            atDay: 8
          },
          {
            id: 'defi-rotation',
            title: 'DeFi yields spike as liquidity exits centralized venues',
            summary: 'UNI and AVAX rally while majors stabilize.',
            drift: 0.09,
            assets: ['UNI', 'AVAX', 'ALT_BASKET'],
            atDay: 19
          },
          {
            id: 'repeg',
            title: 'Stablecoin repegs after audit; relief bounce across majors',
            summary: 'Confidence returns; BTC and ETH surge as spreads normalize.',
            drift: 0.11,
            assets: ['BTC', 'ETH', 'SOL'],
            atDay: 36
          }
  ...
  },
  {
    ...
    scenarios: [
      ...
      },
      {
        ...
        teaser: 'Rumors swirl around a major exchange; liquidity sloshes to DEXs.',
        difficulty: 3,
        segment: 'crypto',
        durationDays: 60,
        bias: -0.0035,
        ...
        xpRewards: [100, 200, 300],
        description:
          'Echoes historic exchange scares: withdrawal pauses, solvency rumors, and rapid shifts to self-custody and DeFi rails.',
        tips: [
          'Monitor exchange-specific headlines; reduce exposure to venues under stress.',
          'Majors may dip first, while DEX and L2 tokens can outperform on migration flows.',
          'Keep some balance in stables and BTC/ETH to redeploy after panic wicks.'
        ],
        news: [
          {
            id: 'withdrawals',
            title: 'Exchange pauses withdrawals citing liquidity checks',
            summary: 'Panic drives BTC and ETH lower; on-chain volumes spike.',
            drift: -0.16,
          ...
          {
            id: 'dex-surge',
            title: 'DEX volumes soar as traders flee centralized venues',
            summary: 'UNI and SOL rally on higher on-chain activity.',
            drift: 0.11,
            assets: ['UNI', 'SOL', 'AVAX'],
            atDay: 20
          },
          {
            id: 'auditor',
            title: 'Auditor steps in; partial reassurance calms majors',
            summary: 'BTC and ETH rebound; smaller caps stay choppy.',
            drift: 0.09,
            assets: ['BTC', 'ETH', 'MNT'],
            atDay: 39
          }
